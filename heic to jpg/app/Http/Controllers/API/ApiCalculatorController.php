...
class ApiCalculatorController extends Controller
    {
        ...
        {
            $rules = [
                'weight_lbs' => 'numeric', // Numeric validation for weight_lbs (optional)
                'weight_kg' => 'numeric', // Numeric validation for weight_kg (optional)
                'height_cm' => 'numeric', // Numeric validation for height_cm (optional)
                'height_in' => 'numeric', // Numeric validation for height_in (optional)
                'height_ft' => 'numeric', // Numeric validation for height_ft (optional)
                'goal' => 'required|string|in:Weight Loss,Mid weight Loss,Extreme Weight Loss,Weight Gain,Mid weight Gain,Extreme Weight Gain',
                'activity-level' => 'required|string|in:Sedentary lifestyle,Light exercise,Moderate exercise,Hard exercise,Physical Job or hard exercise,Professional Job or hard exercise,Professional Athlete',
            ];
            ...
            if ($validator->fails()) {
                return response()->json(['error' => 'Invalid or missing fields in the request.', 'errors' => $validator->errors()], 400);
            }

            // Rest of your existing code for calculations (height and weight conversions, etc.)
